...
bool Camera_Cmd(FunctionalState cmd) {
  ...
  if(cmd == ENABLE) {
    ...
    if(_enabled){
      fprintf(stdout, "already enabled\n");
      return true;
    }
...
void Camera_Record_Snippet(uint32_t timeToRecord) {
  ...
  fprintf(stdout, "Begin Camera Record Snippet at %u.\n", time_ms);
  if(!_initialized) {
     fprintf(stderr, "CAMERA TASK: (record snippet) Camera not initialized, initializing\n");
    Camera_Init();
  }
  if(!_enabled) {
    fprintf(stderr, "CAMERA TASK: (record snippet) Camera not enabled, quitting\n");
    return;
  }
  ...
  fprintf(stderr, "CAMERA TASK: (record snippet) Standby On\n");
  Camera_Enable_Standby(); 
  vTaskDelay(5000);
  fprintf(stderr, "CAMERA TASK: (record snippet) toggling record (on)\n");
  Camera_Toggle_Record();
    // give camera time to start up
  //fprintf(stderr, "CAMERA TASK: (record snippet) Warming up camera\n");
  //vTaskDelay(10000);
  vTaskDelay(timeToRecord + TURN_ON_TIME);
  fprintf(stderr, "CAMERA TASK: (record snippet) toggling record (off)\n");
  Camera_Toggle_Record();
  vTaskDelay(5000);
  fprintf(stderr, "CAMERA TASK: (record snippet) Standby Off\n");
  Camera_Disable_Standby(); 
